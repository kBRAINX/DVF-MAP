+---------------------------------------------------+
## README - Script de Scraping Leboncoin avec Proxy 
+---------------------------------------------------+

Ce script Python (lbc_ws_proxy.py) permet de scraper des annonces immobilières sur Leboncoin, de sauvegarder les données dans une base de données PostgreSQL et dans un fichier Json, de télécharger des images via un proxy Bright Data, afin de générer des graphiques d'analyse de performance.
...
- **Playwright** : Après l'installation, configurez les navigateurs avec :playwright install
...
Remplacez les identifiants dans le fichier **.env** crée plus tot:
    - BRIGHT_DATA_CUSTOMER_ID, BRIGHT_DATA_ZONE, BRIGHT_DATA_PASSWORD pour les proxies HTTP.
    - BRIGHT_DATA_BROWSER_AUTH pour le navigateur Bright Data.
...
### Dossier de sortie :
----------------------
Le script crée un dossier scraped_data pour stocker les JSON, images et captures d'écran.
...
```bash
python lbc_ws_proxy.py "https://leboncoin.fr/annonce/123456"
```
...
### Dépannage
--------------
Erreur de connexion PostgreSQL : Vérifiez les identifiants dans DB_CONFIG et assurez-vous que le serveur est en cours d'exécution.
Erreur Bright Data : Vérifiez les identifiants et l'accès au proxy/navigateur Bright Data.
Timeout Playwright : Augmentez les délais dans page.goto ou vérifiez la connexion réseau.
Images non téléchargées : Vérifiez les permissions d'écriture et l'URL des images.

### Notes
----------
Le script utilise la plateforme de proxy Bright Data par requête (paramètre proxy_count=1).
